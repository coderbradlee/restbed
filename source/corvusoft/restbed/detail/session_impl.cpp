...
namespace restbed
{
    namespace detail
    {
        ...
        void SessionImpl::fetch( const std::shared_ptr< Session > session, const function< void ( const std::shared_ptr< Session > ) >& callback )
        {
            session->m_pimpl->m_request->m_pimpl->m_buffer = std::make_shared< boost::asio::streambuf >( );
            
            m_request->m_pimpl->m_socket->read( session->m_pimpl->m_request->m_pimpl->m_buffer, "\r\n\r\n", std::bind( &SessionImpl::parse_request, this, _1, session, callback ) );
        }
        
        void SessionImpl::fetch_body( const size_t length, const std::shared_ptr< Session > session, const function< void ( const std::shared_ptr< Session >, const Bytes& ) >& callback ) const
        {
        ...
        void SessionImpl::failure( const std::shared_ptr< Session > session, const int status, const exception& error ) const
        {
...
////////////////////////////
        ...
        {
            smatch matches;
            static const std::regex pattern( "^([0-9a-zA-Z]*) ([a-zA-Z0-9:@_~!,;=#%&'\\-\\.\\/\\?\\$\\(\\)\\*\\+]+) (HTTP\\/[0-9]\\.[0-9])\\s*$" );
            string data = "";
            getline( stream, data );
            
            if ( not std::regex_match( data, matches, pattern ) or matches.size( ) not_eq 4 )
            {
        ...
        const multimap< string, string > SessionImpl::parse_request_headers( istream& stream )
        {
            ...
            {
                if ( not std::regex_match( data, matches, pattern ) or matches.size( ) not_eq 3 )
                {
        ...
        void SessionImpl::parse_request( const boost::system::error_code& error, const std::shared_ptr< Session > session, const function< void ( const std::shared_ptr< Session > ) >& callback )
